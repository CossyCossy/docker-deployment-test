        ...
        stage("move-images-hub and start containers") {
            steps {
                    ...
                    script {
                        ...
                        bat "docker run -d --name db --network ${DOCKER_NETWORK} -e POSTGRES_DB=bg -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=admin -p 5432:5432 -v /postgres_data:/var/lib/postgresql/data cossycossy/db"                    }
                   
                    echo 'BG TO HUB'
                    script {
                        bat 'docker push cossycossy/bg:latest'
                        bat "docker run --name bg --network ${DOCKER_NETWORK} -p 8000:8000 -d cossycossy/bg"
                    }
                   
                    echo 'UI TO HUB'
                    script {
                        bat 'docker push cossycossy/front:latest'
                        bat "docker run --name front --network ${DOCKER_NETWORK} -p 80:80 -d cossycossy/front"
                    }
